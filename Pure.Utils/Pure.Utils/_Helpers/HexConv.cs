...
namespace Pure.Utils
{
    ...
    public class HexConv
    {
        ...
        public static ulong X2H(string value, int fromRadix)
        {
            ...
            {
                char schar = value[i];
                if (!baseChar.Contains(schar))
                {
                    throw new ArgumentException(string.Format("参数中的字符\"{0}\"不是 {1} 进制数的有效字符。", schar, fromRadix));
                }

                result += (ulong)baseChar.IndexOf(schar) * (ulong)Math.Pow(baseChar.Length, value.Length - i - 1);
            }
